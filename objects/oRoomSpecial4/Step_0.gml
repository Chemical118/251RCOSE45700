...
if(_boolcheck){
if ( global.KillBoss == true && oPlayer.PlayerSkill == oSkillW)
{
...
else if(oPlayer.PlayerSkill == oSkillG)
{
	ScreenShake(3,30);
	NewTextBox("Kill the boss and talk to NPC to use this thing");
	with(oPlayer)
	{
...
{
	ScreenShake(3,30);
	NewTextBox("Talk to NPC to use this thing");
	with(oPlayer)
	{
		x -= lengthdir_x(5,direction);
		y -= lengthdir_y(5,direction);
