...
else
{
	...
	else if  (AttackFrame != 0)
	{
		...
		if(EnemyReload -  AttackFrame == 45)  sprite_index = Idle;
		AttackFrame--;
	}
	else if( _distance <  ReactionLength && AttackFrame == 0)
	{
		...
		with(instance_create_layer(other.x,other.y,"Instances", ObjectShot))
		{
			...
			direction = 45 * i + other.direction;
	...
	if( AttackLength <  _distance  && _distance < ReactionLength)
	{
		...
		{
			while(!place_meeting(x + sign(_targetX) , y, pEnemyFollow)) x += sign(_targetX);
			_targetX = 0;
		}
		if (place_meeting(x , y + _targetY, pEnemyFollow))
		{
			while(!place_meeting(x , y + sign(_targetY), pEnemyFollow)) y += sign(_targetY);
			_targetY = 0;
		}
